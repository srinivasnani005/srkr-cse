<?php include '_dbconnect.php'; ?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <link rel="stylesheet" href="css/t1.css">
</head>
<body>
    <?php include '_nav.php'; ?>
    <main>
        <div class="background-image">
            <div class="background-content">
                <h1 class="background-title">SRKR Engineering College</h1>
                <p class="background-text">To emerge as a world-class technical institution that strives for the socio-ecological well-being of society.</p>
                <a href="#" class="download-btn">NAAC Grade</a>
                <a href="#" class="whats-new-btn">What's New</a>
            </div>
        </div>

        <div class="body-content"></div>
    </main>

    <?php include '_modal.php'; ?>
    <!-- whats new component here  -->
    <?php include '_footer.php'; ?>

    <script src="js/t1.js" defer></script>
</body>
</html>
