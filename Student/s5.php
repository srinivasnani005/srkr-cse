...
$academicYear = $eventType = $eventTitle = $numberOfDays = $eventStartDate = $eventEndDate = $organizedBy = $eventLevel = $eventFees = $participationCertificate = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $numberOfDays = test_input($_POST['number_of_days']);
    $eventStartDate = test_input($_POST['event_start_date']);
    $eventEndDate = test_input($_POST['event_end_date']);
    $organizedBy = test_input($_POST['organized_by']);
    $eventLevel = test_input($_POST['event_level']);
    $eventFees = test_input($_POST['event_fees']);
    ...
    $targetDir = "../uploads/S5/$branch/2024/";
    $participationCertificate = $targetDir . basename($_FILES["participation_certificate"]["name"]);

    if (move_uploaded_file($_FILES["file_name"]["tmp_name"], $participationCertificate)) {
        // Database connection
    

        // Check for duplicate data before inserting
        $checkDuplicate = "SELECT * FROM S5 WHERE event_title = '$eventTitle'";
        ...
        if ($result && $result->num_rows > 0) {
            ...
            $sql = "INSERT INTO S5 (academic_year, event_type, event_title, number_of_days, event_start_date, event_end_date, organized_by, event_level, event_fees, participation_certificate)
                    VALUES ('$academicYear', '$eventType', '$eventTitle', '$numberOfDays', '$eventStartDate', '$eventEndDate', '$organizedBy', '$eventLevel', '$eventFees', '$participationCertificate')";
...
<head>
    ...
    <title>S5 Student Workshops/Seminars Participated</title>
    <style>
...
<body>
    ...
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" id="form-3" enctype="multipart/form-data">
        <!-- Form fields for S5 Student Workshops/Seminars Participated -->
        ...
        <div class="login-box">
            <div class="heading">
                <h3>S5 Student Workshops/Seminars Participated</h3>        
            </div>
        </div>
        ...
        <div class="login-box">
            <div class="user-box">
                <input type="text" name="number_of_days" required="">
                <label>No of days<div id="must">*</div></label>
            </div>
        </div>
        ...
        <div class="login-box">
            <div class="user-box">
                <input type="text" name="event_fees" required="">
                <label>Fees if any<div id="must">*</div></label>
            </div>
        </div>

        <!-- Attach Participation Certificate -->
        <div class="login-box">
            <div class="user-box">
                <input type="file" name="participation_certificate">
                <label>Attach participation certificate<div id="must">*</div></label>
            </div>
        </div>
